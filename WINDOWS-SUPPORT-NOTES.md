...
## 2026-08-23 iteration 33

- Revalidated the Windows-support delivery gates without changing product files.
- Validation: `bash tests/fm-session-lock-ancestry.test.sh` passed with 8 ok lines.
- Validation: `bash tests/fm-opencode-plugins.test.sh` passed.
- Validation: `bash tests/fm-supervision-instructions.test.sh` passed.
- Validation blocked: `bash bin/fm-lint.sh` stopped because ShellCheck 0.11.0 is unavailable on `PATH`.
...
## 2026-08-23 iteration 38

- Revalidated the Windows-support delivery gates.
- Validation: `bash tests/fm-session-lock-ancestry.test.sh` passed with 8 ok lines when allowed sufficient runtime.
- Validation: `bash tests/fm-opencode-plugins.test.sh` passed.
- Validation: `bash tests/fm-supervision-instructions.test.sh` passed.
- Validation blocked: `bash bin/fm-lint.sh` stopped because ShellCheck 0.11.0 is unavailable on `PATH`.
...
- Validation: `bash tests/fm-supervision-instructions.test.sh` passed, including MSYS OpenCode detection.
- Validation blocked: `bash bin/fm-lint.sh` stopped because ShellCheck 0.11.0 is unavailable on `PATH`.
- Local probes found no ShellCheck installation through `PATH`, winget, Chocolatey, Scoop, or WSL.
- `WINDOWS-SUPPORT-PR.txt` remains absent, so the stop condition is not met.

## 2026-08-23 iteration 48
...
- Validation: `bash tests/fm-supervision-instructions.test.sh` passed, including MSYS OpenCode detection.
- Validation blocked: `bash bin/fm-lint.sh` stopped before analysis because ShellCheck 0.11.0 is unavailable on `PATH`.
- Documentation checks confirm `docs/windows.md` contains `Install`, `Runtime backend`, and `Known limits`, and `README.md` links the guide.
- `WINDOWS-SUPPORT-PR.txt` remains absent, so the stop condition is not met.

## 2026-08-23 iteration 57
